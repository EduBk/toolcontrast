...
function Layout() {
  ...
  const wcagCompliance = getWCAGCompliance();
  let ratioLetter;
  ...
  if (contrastRatio < 4.5) {
    ...
    ratioLetter = "very good";
  }

  return (
    <main className="flex flex-col w-full items-center">
      <section className="flex w-4/6 h-28 overflow-y-auto items-center justify-between sticky top-0 bg-slate-50 z-50">
        <span className="font-bold mr-3">Elements</span>
        <div className="flex gap-5 text-nowrap ml-3 mr-3">
          <div className="flex flex-col">
            <span className="text-xs">Ratio Contrast</span>
            <span
              className={`text-sm font-bold ${
                contrastRatio <= 4.5 ? "text-red-500" : "text-green-500"
              }`}
            >
              {contrastRatio} {ratioLetter}
            </span>
          </div>
          <div className="flex flex-col">
            ...
            <div className="flex gap-6 justify-center">
              <span
                className={`text-sm font-bold ${
                  wcagCompliance != "Fail" ? "text-green-500" : "text-red-500"
                }`}
              >
                AA
              </span>
              <span
                className={`text-sm font-bold ${
                  wcagCompliance == "AAA" ? "text-green-500" : "text-red-500"
                }`}
              >
                AAA
              </span>
        ...
        <div className="flex gap-4 ml-3 mr-3 justify-center items-center">
          <ColorPicker isTextColor={true} />
          <Image
            className="dark:invert w-4 h-5 cursor-pointer"
            src="https://img.icons8.com/fluency-systems-filled/50/back-and-forth.png"
            alt="Tool Logo"
            width={50}
            height={50}
            priority
      ...
      <footer className="w-4/6">
        <div className="flex justify-between items-center mt-6 ">
          <span className="font-bold text-sm">Eduardo Martinez</span>
          <svg
